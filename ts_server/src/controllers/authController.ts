...
export const sign = async (req: Request, res: Response): Promise<void> => {
  ...
  bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
    if (err) {
      ...
      const query = "INSERT INTO user (id, pw, name, phone_number) VALUES (?,?,?,?);";
      const CreateUser = [id, hashedPassword, username, phoneNumber];

      connection.query(query, CreateUser, (err: mysql.MysqlError | null) => {
        if (err) {
          console.log(err);
        } else {
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    connection.query(query, [id], async (err: mysql.MysqlError | null, result: User[]) => {
      if (err) {
        console.error(err);
        res.status(500).json({ status: 500, message: '로그인에 실패했습니다.' });
        return;
      }

      if (result.length === 0) {
        res.status(403).json({ status: 403, message: '아이디를 찾을 수 없습니다.' });
        return;
      }
      ...
      if (!match) {
        res.status(403).json({ status: 403, message: '비밀번호가 일치하지 않습니다.' });
        return;
      }
...
export const loginSuccessfully = (req: Request, res: Response): void => {
...
export const managerUserList = async (req: Request, res: Response): Promise<void> => {
  const query = `SELECT u.id, u.name, DATE_FORMAT(u.join_date, '%Y-%m-%d') as join_date, IFNULL(COUNT(r.name), 0) as count 
    FROM user u LEFT JOIN report r ON u.name = r.name
    GROUP BY u.id, u.name, join_date
    ORDER BY count DESC;`;

  connection.query(query, (err: Error, result: User[]) => {
    if (err) {
      console.log(err);
    } else {
      res.json(result);
